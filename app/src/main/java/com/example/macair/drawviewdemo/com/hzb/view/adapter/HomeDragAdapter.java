...
public class HomeDragAdapter extends BaseAdapter implements DragGridBaseAdapter{
	
	private List<HashMap<String, Object>> list;
	private LayoutInflater mInflater;
	private int mHidePosition = -1;
	
	public HomeDragAdapter(Context context, List<HashMap<String, Object>> list){
		this.list = list;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		Integer id =(Integer) list.get(position).get("item_image");
		if (id != null){
		   mImageView.setImageResource(id);
		   mTextView.setText((CharSequence) list.get(position).get("item_text"));
		   if(id==R.drawable.home_item_more)
